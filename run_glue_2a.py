...
""" Finetuning the library models for sequence classification on GLUE (Bert, XLM, XLNet, RoBERTa).
    Distributed training with gradient synchronization via gather/scatter (Part 2a).
"""
...
def synchronize_gradients(model, args):
    """
    Gather gradients from all workers onto rank 0, compute element-wise mean,
    then scatter the averaged gradient back to all workers.
    """
    ...
    for param in model.parameters():
        ...
        # ------------------------------------------------------------------
        # GATHER: rank 0 collects gradients from every worker
        # ------------------------------------------------------------------
        if rank == 0:
        ...
        # ------------------------------------------------------------------
        # AVERAGE + SCATTER: rank 0 averages and sends result to all workers
        # ------------------------------------------------------------------
        if rank == 0:
        ...
        # Every worker receives the averaged gradient into a fresh buffer,
        # then we write it back into param.grad so the optimizer uses it.
        recv_buf = torch.zeros_like(grad)
        dist.scatter(recv_buf, scatter_list=scatter_list, src=0)
        param.grad.data.copy_(recv_buf)
